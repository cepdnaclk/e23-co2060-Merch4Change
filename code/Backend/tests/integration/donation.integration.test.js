...
async function runTests() {
  ...
  let verifiedCharity;
  await assertTest(
    "1. GET /api/v1/donations/charities returns verified charities",
    async () => {
      const res = await fetch(`${baseUrl}/api/v1/donations/charities`);
      const json = await res.json();
      if (
        !json.success ||
        !Array.isArray(json.data.charities) ||
        json.data.charities.length === 0
      ) {
  ...
  let targetProject;
  await assertTest(
    "2. GET /api/v1/donations/projects returns active projects under verified charities",
    async () => {
      ...
      targetProject =
        json.data.projects.find((p) => p.charityId === verifiedCharity._id) ||
  ...
  const initialProjectCollected =
    (await Project.findById(targetProject.id || targetProject._id))
      ?.collectedAmount || 0;
  ...
  await assertTest(
    "3. POST /api/v1/donations completes donation, deducts coins, and updates project",
    async () => {
      ...
      if (res.status !== 201 || !json.success) {
        throw new Error(
          `Expected 201, got ${res.status}: ${JSON.stringify(json)}`,
        );
      }
      if (json.data.coinBalance !== initialCoins - donationAmount) {
        throw new Error(
          `Expected remaining coins ${initialCoins - donationAmount}, got ${json.data.coinBalance}`,
        );
      }
      ...
      const updatedProject = await Project.findById(
        targetProject.id || targetProject._id,
      );
      if (
        updatedProject.collectedAmount !==
        initialProjectCollected + donationAmount
      ) {
        throw new Error(
          `Project collectedAmount was not incremented. Expected ${initialProjectCollected + donationAmount}, got ${updatedProject.collectedAmount}`,
        );
      }
  ...
  await assertTest(
    "4. POST /api/v1/donations rejects donation when coins are insufficient",
    async () => {
      const res = await fetch(`${baseUrl}/api/v1/donations`, {
        method: "POST",
        headers: {
      ...
      if (res.status !== 400 || json.error?.code !== "INSUFFICIENT_COINS") {
        throw new Error(
          `Expected 400 INSUFFICIENT_COINS, got ${res.status}: ${JSON.stringify(json)}`,
        );
      }
    },
  );
  ...
  await assertTest(
    "5. POST /api/v1/donations rejects unverified charities",
    async () => {
      ...
      if (res.status !== 403 || json.error?.code !== "CHARITY_NOT_VERIFIED") {
        throw new Error(
          `Expected 403 CHARITY_NOT_VERIFIED, got ${res.status}: ${JSON.stringify(json)}`,
        );
      }
    },
  );
  ...
  await assertTest(
    "6. GET /api/v1/donations/my returns paginated user history with populated info",
    async () => {
      const res = await fetch(
        `${baseUrl}/api/v1/donations/my?page=1&limit=10`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (
        !json.success ||
        !Array.isArray(json.data.donations) ||
        json.data.donations.length === 0
      ) {
        throw new Error(
          `Failed to fetch my donations: ${JSON.stringify(json)}`,
        );
      }
      const latestDonation = json.data.donations[0];
      if (
        !latestDonation.charity ||
        !latestDonation.project ||
        !latestDonation.amount ||
        !latestDonation.status
      ) {
        throw new Error(
          `Donation object missing mapped fields: ${JSON.stringify(latestDonation)}`,
        );
      }
    },
  );
  ...
  await assertTest(
    "7. GET /api/v1/donations/my/stats calculates real impact metrics and ongoing projects",
    async () => {
      ...
      if (!json.success)
        throw new Error(
          `Failed to fetch donation stats: ${JSON.stringify(json)}`,
        );
      const {
        totalDonated,
        causesSupported,
  ...
  await assertTest(
    "8. GET /api/v1/leaderboards/donors ranks donors using unified Donation records",
    async () => {
      const res = await fetch(
        `${baseUrl}/api/v1/leaderboards/donors?timeframe=all_time&limit=10`,
      );
      const json = await res.json();
      if (
        !json.success ||
        ...
        throw new Error(
          `Failed to fetch donor leaderboard: ${JSON.stringify(json)}`,
        );
      }
      const topDonor = json.data.leaderboard[0];
      if (topDonor.rank !== 1 || !topDonor.totalCoins || !topDonor.tier) {
        throw new Error(
          `Invalid top donor structure: ${JSON.stringify(topDonor)}`,
        );
      }
    },
  );
  ...
  await assertTest(
    "9. GET /api/v1/leaderboards/stats aggregates community donation totals",
    async () => {
      const res = await fetch(`${baseUrl}/api/v1/leaderboards/stats`);
      const json = await res.json();
      if (!json.success)
        throw new Error(
          `Failed to fetch leaderboard stats: ${JSON.stringify(json)}`,
        );
      if (
        typeof json.data.totalCoinsDonated !== "number" ||
        json.data.totalCoinsDonated <= 0
      ) {
        throw new Error(
          `Invalid totalCoinsDonated: ${json.data.totalCoinsDonated}`,
        );
      }
    },
  );
  ...
  await assertTest(
    "10. GET /api/v1/profile/me/coins reflects live remaining coins",
    async () => {
      ...
      if (
        !json.success ||
        json.data.coinBalance !== donorUser.coinBalance - donationAmount
      ) {
        throw new Error(
          `Expected coinBalance ${donorUser.coinBalance - donationAmount}, got ${json.data?.coinBalance}`,
        );
      }
    },
  );
